...
async def _poll_nxt_spread(broadcast, w: "_Watch") -> None:
    ...
    while time.time() < w.expire:
        # VI 해제(연속매매 재개)·KRX 첫체결 후엔 단일가 차익 종료 → 폴링 중단
        if w.released or w.krx_resumed:
            break
        ...
        exp = round(_num(krx.get("exp_cntr_pric")))                    # 단일가 예상체결가
        single_price = exp > 0                                          # 진짜 단일가 구간 여부
        krx_exp = exp if single_price else round(_num(krx.get("cur_prc")))  # 폴백=현재가(스테일)
        if nxt_ask > 0 and krx_exp > 0:
            spread = krx_exp - nxt_ask
            cost = krx_exp * TAX + (nxt_ask + krx_exp) * FEE
            net = spread - cost
            # opportunity = 진짜 단일가(exp>0) + 발동 초기 윈도우 내 + 순이익 > 비용버퍼
            opportunity = (single_price and sec_since_vi <= _OPP_WINDOW_SEC
                           and net > cost * (EDGE_BUFFER - 1))
            # first_buy 는 첫 폴링틱이 아니라 '첫 기회 시점' 매수가로 고정
...
_OPP_WINDOW_SEC = 50   # 발동 후 이 시간 내 + 단일가(exp>0) 구간만 opportunity 인정 (이후는 수렴/스테일)
